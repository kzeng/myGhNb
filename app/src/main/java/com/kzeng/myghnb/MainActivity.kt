...
fun MyGhNbApp() {
    ...
    Scaffold(
        ...
        when (screen) {
            ...
            "editor" -> EditorScreen(selected ?: Note("", "", ""), padding, onSave = {
...
private fun AboutScreen(context: Context, padding: androidx.compose.foundation.layout.PaddingValues) {
    ...
    Column(Modifier.fillMaxSize().padding(padding).padding(28.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        Image(painterResource(com.kzeng.myghnb.R.drawable.ic_launcher_logo), "My GH Notebook", Modifier.size(112.dp))
        Spacer(Modifier.height(18.dp))
...
private object GitHubClient {
    ...
    private fun connection(context: Context, url: String): HttpURLConnection = (URL(url).openConnection() as HttpURLConnection).apply {
        ...
        context.getSharedPreferences("settings", 0).getString("github_token", null)?.let { setRequestProperty("Authorization", "Bearer $it") }
    }
    fun fetchNotes(context: Context): List<Note> = runCatching {
        val c = connection(context, "$repo?ref=clash"); val array = JSONArray(c.inputStream.bufferedReader().readText())
        List(array.length()) { i ->
            val o = array.getJSONObject(i)
            val raw = o.optString("download_url").takeIf { it.isNotBlank() }?.let { url -> runCatching { URL(url).openStream().bufferedReader().readText() }.getOrDefault("") } ?: ""
